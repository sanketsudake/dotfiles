...
3. Enter each day's hours with **`fill --by cell`** — addresses the input by its **day column header** and *replaces* the cell's `0` (not appends → `80`), so no per-day `snap` for input names, no session-specific ids: `chrome-cdp fill --by cell "Mon, 7/13" "8" --json`, repeated per weekday (day headers come from the grid, e.g. `Mon, 7/13` … `Fri, 7/17`).
   ...
   Requires chrome-cdp ≥ 0.2.2: earlier builds failed the Tue column reproducibly as `occluded` (the resolver picked Workday's global "Search Workday" box, which shares that column's x-range), and 0.2.2 ranks the header's own grid first and names the cover in the error message.
   **If a column still fails as `occluded`**, read the message — it names what covers the cell — then `chrome-cdp value --all "input[data-automation-id=numericInput]" --json` shows which cell is still `0`; the fallback that always works is to read that input's DOM id with `eval` (`[...document.querySelectorAll('input[data-automation-id=numericInput]')][2].id`) and `chrome-cdp fill '[id="<that id>"]' "8" --json`.
