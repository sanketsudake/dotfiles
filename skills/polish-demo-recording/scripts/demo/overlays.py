...
def build_ass(ctx, tl, tm, captions=False):
    ...
    if ctx.strip:
        for o1, o2, a in runs:
            lines.append(ev(2, o1, o2, 'Hdr', f'{{\\an4\\pos(40,{ctx.strip//2})\\1c&H{ctx.accent_ass}&}}{ctx.brand["name"]}{{\\1c&HB8C0CC&\\fs26}}   {ctx.brand.get("subtitle", "Product demo")}'))
    ...
    for i, (t, text) in enumerate(callouts):
        ...
        lines.append(ev(4, o1, o2, 'Box', fad + rect(x1, y1, x1 + 10, y2, ctx.accent_ass)))
    ...
    for s in tl:
        if s['kind'] == 'src' and s['badge']:
            ...
            w = tw(txt, ctx.fb, 26) + 44
            x2, y2 = ctx.W - 60, ctx.H - 40
            x1, y1 = int(x2 - w), y2 - 48
