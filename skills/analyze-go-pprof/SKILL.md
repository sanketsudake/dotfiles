...
description: >-
  Pulls the heap/goroutine pprof profiles a CI job captured, separates a real
  leak from baseline cost, and quantifies a fix's before/after delta. Use for
  "where is the memory/compute going", "is this a leak", "verify the perf
  fix", "compare profiles from the latest CI run", or when a Go service's CI
  run uploads pprof artifacts. Generic to any Go project whose CI captures
  /debug/pprof dumps.
license: Apache-2.0
metadata:
  author: sanketsudake
...
CI captures `/debug/pprof/heap` and `/debug/pprof/goroutine?debug=1` into an artifact, gated by a `pprof.enabled`-style flag — dumps exist only on flagged runs.
Check the project's `CLAUDE.md` for the artifact name and covered services.
...
- `-inuse_space` shows live heap at capture (steady-state footprint); heap captures use inuse, not `-alloc_space` (cumulative churn).
- Also run `-cum`: the flat top is often a leaf (`zapcore.newCounters`, `<pkg>.init`); `-cum` names the allocating caller.
...
A leaked goroutine shows a high count on one frame that should be 1-per-something — e.g. a pool worker: 44 leaked, 1 after the fix.
...
- **Constant init cost = baseline; monotonic growth = leak.**
  A package-scope `var log = GetLogger()` or scheme/protobuf init allocates once and stays constant — leave it.
...
- **A `SummaryVec` keeps a per-series quantile stream; cost scales with active label combinations.**
  A histogram uses fixed buckets instead — cheaper and aggregatable across replicas.
- **A small CI cluster hides scale-dependent costs.**
  Absence in the profile ≠ absence at scale — reason about cardinality/pod-count separately.

## Quantify a fix: before/after

Download an artifact from a run before the fix and one after.
Compare **composition, not raw totals** — totals drift run-to-run with cluster activity:
...
- **No pprof artifact on the run?**
  The branch built without the profiling flag, or path filters skipped the job.
- **Capture is best-effort.**
  ...
  Recovery: pull the same files from an earlier round of the same PR with identical code (confirm the commit delta is docs/workflow-only), or from another leg.
- Cross-check against a Prometheus TSDB dump (see the `analyze-prometheus-tsdb` skill) to separate a constant offset from growth over time.
- **Histogram bucket choice matters**: a lifetime metric (seconds→hours) needs `ExponentialBuckets`, not `DefBuckets` (tops out at 10s, dumps everything into `+Inf`).
