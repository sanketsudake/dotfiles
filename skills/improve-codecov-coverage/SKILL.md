...
description: >-
  Raises test coverage on a Go project that reports to Codecov: fetches
  totals, ranks low-covered packages, writes targeted tests, and verifies
  the delta. Use when the user says "improve code coverage", "cover
  package X", or "find coverage gaps". Generic to any Go + Codecov
  project.
license: Apache-2.0
metadata:
  author: sanketsudake
...
Use the `superpowers:using-git-worktrees` skill if available.
...
The `totals` endpoint combines all files across all CI upload flags.
It is the source of truth for gap analysis.
For the Python snippet that prints top-level totals and the JSON response shape, see `references/codecov-recipes.md`.
...
Most setups use two upload flags: `unittests` (from `go test -coverprofile`) and `integration`.
Codecov merges them into the combined number shown in the UI.
A local unit-only run is therefore lower than the Codecov number.
Integration coverage can come from a server binary built with `-cover` and `GOCOVERDIR`, collected from running pods, and merged:
...
3. Check the project's CI workflow to see if integration tests run the CLI/library in-process.
   If yes, one integration test can cover both the command handler and the server-side code.
...
The `integration` flag can lag 5-15 minutes after CI finishes before it appears in Codecov.
...
| Mistake | Fix |
|---|---|
| Using local unit coverage as the gap analysis baseline | Fetch Codecov totals first. Local unit-only coverage can be far lower than the Codecov combined number. |
| Checking Codecov immediately after CI finishes | Poll the API as shown in "Iterate and Verify". The `integration` flag can lag after CI finishes. |
