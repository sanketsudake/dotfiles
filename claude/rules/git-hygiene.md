...
- A `PreToolUse` hook (`scripts/safety-guard-hook.py`, wired per-profile in `settings.json`) hard-denies `git add -A|--all|.`, `sudo`, `chmod/chown 777`, and `rm -rf` on `/`, `~`, `$HOME`, `.git`; asks before any recursive `rm`, force-push, `reset --hard`, or `clean -f`; and denies `Edit|Write` to `.env*` (except `.env.example`-style templates), `.git/`, `node_modules/`.
