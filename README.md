...
- **mise** keeps Omarchy's `claude`/`codex`/`gh` CLIs, but not Node.js: node comes from nix on both hosts, so run `mise rm -g node` after an Omarchy install (and after `omarchy-install-dev-env`, which adds it back); `05-omarchy.zsh` activates it ahead of the nix profile and ports the rest of Omarchy's bash rc (env bootstrap, editor/browser exports, starship, eza aliases). The module is a no-op on macOS.
